...
def get_face_embedding1(face_net, face_detect, files_list, names_list):
    '''
    获得embedding数据
    :param files_list: 图像列表
    :param names_list: 与files_list一一的名称列表
...
def create_face_embedding(model_path,dataset_path,out_emb_path,out_filename):
    ...
    :return: None
    '''
    files_list,names_list=file_processing.gen_files_labels(dataset_path,postfix=['*.jpg', '*.png'])
...
def upload(img_path):
    url = 'http://gd.aigoes.com/gd-smz-admin/fastdfs/fastDFS/fastDFSUpload'  # 接口地址
...
def create_face_embedding1(face_net, face_detect,dataset_path,out_emb_path,out_filename):
    '''
    ...
    '''
    files_list,names_list=file_processing.gen_files_labels(dataset_path,postfix=['*.jpg', '*.png'])
    embeddings,label_list=get_face_embedding1(face_net, face_detect,files_list, names_list)
    print("label_list:{}".format(label_list))
    print("have {} label".format(len(label_list)))
...
if __name__ == '__main__':
    ...
    dataset_path='unknow/images'
    out_emb_path = 'unknow/emb/faceEmbedding.npy'
    out_filename = 'unknow/emb/name.txt'
